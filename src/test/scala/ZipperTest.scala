...
class ZipperTest extends AnyFunSuite with Matchers:
   ...
   test("setLeft with Some") {
      pending
      z.toTree(z.setLeft(Some(BinTree(5, None, None)), fromSome(z.left(z.fromTree(t1))))) should be(t3)
   }
   ...
   test("different paths to same zipper") {
      pending
      z.right(fromSome(z.up(fromSome(z.left(z.fromTree(t1)))))) should be(z.right(z.fromTree(t1)))
   }
