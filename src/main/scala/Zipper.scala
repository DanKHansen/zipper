...
object Zipper:
   case class Zipper[A](focus: BinTree[A], up: Option[Zipper[A]], isLeft: Boolean)
   def fromTree[A](bt: BinTree[A]): Zipper[A] = Zipper(bt, None, false)
   @tailrec
   ...
   def right[A](zipper: Zipper[A]): Option[Zipper[A]] = zipper.focus.right.map(ch => Zipper(ch, Some(zipper), false))
